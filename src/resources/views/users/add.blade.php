												...
												<form id="loginForm" method="POST" action="{{ route("user.create.post") }}" class="form-horizontal">
																...
																<div class="card">
																				...
																				<div class="card-body">
																								...
																								<div class="form-group row">
																												<label class="control-label col-sm-2 align-self-center mb-0" for="password">Password: <span
																																				data-bs-toggle="tooltip"
																																				data-bs-original-title="Password must contain atleast one upper case letter, one lower case letter, one number, one special character and should be minimum of 8 characters"><i
																																								class="icon">
																																								<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 12 12' width='12'
																																												height='12' fill='none' stroke='currentColor'>
																																												<circle cx='6' cy='6' r='4.5' />
																												...
																												<div class="col-sm-10">
																																<div class="input-group mb-3">
																																				<input type="password" class="form-control" id="password" name="password"
																																								aria-describedby="password" aria-describedby="password-show">
																																				<div class="input-group-append">
																																								<button class="btn btn-light input-group-text" type="button" id="password-show">
																																												<svg width="20px" height="20px" viewBox="0 0 24 24" fill="none"
																																																...
																																																<path d="M2 11.98C8.09 1.31996 15.91 1.32996 22 11.98" stroke="#000000"
																																																				stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
																																																<path d="M22 12.01C15.91 22.67 8.09 22.66 2 12.01" stroke="#000000"
																																																				stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
																																												</svg>
																																								</button>
																																				</div>
																																</div>
																																<div id="password_error" class="invalid-message">
																																				@error("password")
																																								{{ $message }}
																																				@enderror
																																</div>
																								...
																								<div class="form-group row">
																												<label class="control-label col-sm-2 align-self-center mb-0" for="confirm_password">Confirm
																																Password: <span data-bs-toggle="tooltip"
																																				data-bs-original-title="It must be same as the password given above"><i
																																								class="icon">
																																								<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 12 12' width='12'
																																												height='12' fill='none' stroke='currentColor'>
																																												<circle cx='6' cy='6' r='4.5' />
																												...
																												<div class="col-sm-10">
																																<div class="input-group mb-3">
																																				<input type="password" class="form-control" id="confirm_password"
																																								...
																																								aria-describedby="confirm_password-show">
																																				<div class="input-group-append">
																																								<button class="btn btn-light input-group-text" type="button"
																																...
																																<div id="confirm_password_error" class="invalid-message">
																												...
																												<div class="col-sm-10">
																																<select id="role" name="role" class="form-select shadow-none">
																																				...
																																				<option value="Staff-Admin"
																																								{{ !empty(old("role")) && old("role") == "Staff-Admin" ? "selected" : "" }}>Admin
																																				</option>
																																				<option value="Writer"
																																								{{ !empty(old("role")) && old("role") == "Writer" ? "selected" : "" }}>Writer
																																				</option>
																																				<option value="Client"
																																								{{ !empty(old("role")) && old("role") == "Client" ? "selected" : "" }}>Client
																																				</option>
																																</select>
																																@error("role")
																																				<div class="invalid-message">{{ $message }}</div>
																								...
																								<div id="client_div" class="form-group row {{ old("role") === "Client" ? "d-flex" : "d-none" }}">
																												...
																												<div class="col-sm-10">
																																<select id="client" name="client" class="form-select shadow-none">
																																				<option value="" {{ empty(old("client")) ? "selected" : "" }}>Select Client
																																				</option>
																																				@foreach ($client as $item)
																																								<option value="{{ $item->id }}"
																																												{{ !empty(old("client")) && old("client") == $item->id ? "selected" : "" }}>
																																												{{ $item->name }}</option>
																																				@endforeach
																																</select>
																																@error("client")
																												...
																												<div class="col-sm-10">
																																<select id="timezone" name="timezone" class="form-select shadow-none">
																																				...
																																				<option value="Pacific/Honolulu GMT-10:00"
																																								{{ !empty(old("timezone")) && old("timezone") == "Pacific/Honolulu GMT-10:00" ? "selected" : "" }}>
																																								Pacific/Honolulu GMT-10:00</option>
																																				<option value="America/Anchorage GMT-9:00"
																																								{{ !empty(old("timezone")) && old("timezone") == "America/Anchorage GMT-9:00" ? "selected" : "" }}>
																																								America/Anchorage GMT-9:00</option>
																																				<option value="America/Los_Angeles GMT-8:00"
																																								{{ !empty(old("timezone")) && old("timezone") == "America/Los_Angeles GMT-8:00" ? "selected" : "" }}>
																																								America/Los_Angeles GMT-8:00</option>
																																				<option value="America/Boise GMT-7:00"
																																								{{ !empty(old("timezone")) && old("timezone") == "America/Boise GMT-7:00" ? "selected" : "" }}>
																																								America/Boise GMT-7:00</option>
																																				<option value="America/Denver GMT-7:00"
																																								{{ !empty(old("timezone")) && old("timezone") == "America/Denver GMT-7:00" ? "selected" : "" }}>
																																								America/Denver GMT-7:00</option>
																																				<option value="America/Phoenix GMT-7:00"
																																								{{ !empty(old("timezone")) && old("timezone") == "America/Phoenix GMT-7:00" ? "selected" : "" }}>
																																								America/Phoenix GMT-7:00</option>
																																				<option value="America/Chicago GMT-6:00"
																																								{{ !empty(old("timezone")) && old("timezone") == "America/Chicago GMT-6:00" ? "selected" : "" }}>
																																								America/Chicago GMT-6:00</option>
																																				<option value="America/Detroit GMT-5:00"
																																								{{ !empty(old("timezone")) && old("timezone") == "America/Detroit GMT-5:00" ? "selected" : "" }}>
																																								America/Detroit GMT-5:00</option>
																																				<option value="America/New_York GMT-5:00"
																																								{{ !empty(old("timezone")) && old("timezone") == "America/New_York GMT-5:00" ? "selected" : "" }}>
																								...
																								<div id="billing_rate_div"
																												class="form-group row {{ old("role") === "Client" || old("role") === "Writer" ? "d-flex" : "d-none" }}">
																												...
																												<div class="col-sm-10">
																																<select class="form-select shadow-none" id="tool" name="tool[]" multiple>
																																				@foreach ($tool as $item)
																																								<option value="{{ $item->id }}"
																																												{{ collect(old("tool"))->contains($item->id) ? "selected" : "" }}>
																																												{{ $item->name }}</option>
																																				@endforeach
																																</select>
																												</div>
																...
																buttons: [
																				['<button><b>YES</b></button>', async function(instance, toast) {
																								...
																								try {
																												...
																												if (document.getElementById('role').value == 'Client') {
																																formData.append('client', document.getElementById('client').value)
																																formData.append('billing_rate', document.getElementById('billing_rate')
																																				.value)
																												}
																												if (document.getElementById('role').value == 'Writer') {
																																formData.append('billing_rate', document.getElementById('billing_rate')
																																				.value)
																																if (document.getElementById('tool')?.length > 0) {
																																				for (let index = 0; index < document.getElementById('tool')
																																								.length; index++) {
																																								if (document.getElementById('tool')[index].selected) {
																																												if (document.getElementById('tool')[index].value != "") {
																																																formData.append('tool[]', document.getElementById('tool')[
																																																				index].value)
																																												}
																																								}
																																				}
